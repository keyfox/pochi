...
import {
  ATTRS,
  CHARS_TO_DETERMINISTIC_KEYSTROKES_LIST,
  DETERMINISTIC_KEYSTROKES_TO_CHARS,
  getNextKeysequencesList as getNextKeyseqsList,
  interpretKeystrokes,
  sequenceChooser,
} from "../src/msime-ja";
...
describe("msime-ja", () => {
  describe("DETERMINISTIC_KEYSEQUENCE_TO_CHARS", () => {
    function expectToBeDeterministic(arr) {
  ...
  describe("sequenceChooser", () => {
    const candidates_u = getNextKeyseqsList("う");
    const defaultChooser = sequenceChooser();
    ...
    it("choose preffered one", () => {
      ...
      const shiChooser = sequenceChooser({ byChars: { し: "shi" } });
      expect(shiChooser(candidates_shi)).to.have.property("strokes", "shi");
      const siChooser = sequenceChooser({ byChars: { し: "si" } });
    ...
    it("choose preferred one even if inefficient", () => {
      for (let strokes of ["u", "wu", "whu"]) {
        const preferredChooser = sequenceChooser({ byChars: { う: strokes } });
    ...
    it("keeps the reference of given preferences object", () => {
      ...
      const preferredChooser = sequenceChooser(prefs);
    ...
    it("respects single N preference", () => {
      ...
      const chosenAcceptive = sequenceChooser({ acceptSingleN: true })(candidates_nda);
      expect(chosenAcceptive).to.have.property("chars", "んだ");
      expect(chosenAcceptive).to.have.property("strokes", "nda");

      const chosenUnacceptive = sequenceChooser({ acceptSingleN: false })(candidates_nda);
    ...
    it("respects dependent consonants preference", () => {
      ...
      const chosenAcceptive = sequenceChooser({ acceptDependentConsonant: true })(candidates_tte);
      expect(chosenAcceptive).to.have.property("chars", "って");
      expect(chosenAcceptive).to.have.property("strokes", "tte");

      const chosenUnacceptive = sequenceChooser({ acceptDependentConsonant: false })(candidates_tte);
    ...
    it("respects preferred keysequence even in dependent consonants", () => {
      ...
      expect(sequenceChooser({ ...basePrefs, byChars: { し: "shi" } })(candidates_sshi)).to.have.property(
        "strokes",
        "sshi"
      );
      expect(sequenceChooser({ ...basePrefs, byChars: { し: "si" } })(candidates_sshi)).to.have.property(
        "strokes",
        "ssi"
      );
    });
  ...
  describe("resolveKeystrokes", () => {
    ...
    for (let testCase of TEST_CASES) {
      ...
      it(`resolves "${src}" => "${resolvers.map((e) => e[1]).join("")}"${
        ...
        expect(interpretKeystrokes(src)).to.deep.equal({
          resolvers: resolvers.map(([strokes, chars, attrs]) => ({ strokes, chars, attrs })),
          pending,
        });
      });
    }
